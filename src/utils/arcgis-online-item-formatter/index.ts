...
export const formatAsAgolItem = (
    item: AgolItem,
    { agolHost = 'https://www.arcgis.com', thumbnailWidth = 200 } = {}
) => {
    if (!item) {
        return null;
    }
...
export const isSubscriberContent = (typeKeywords: string[]) => {
    return typeKeywords.indexOf('Requires Subscription') >= 0 ? true : false;
};

export const isPremiumContent = (typeKeywords: string[]) => {
    return typeKeywords.indexOf('Requires Credits') >= 0 ? true : false;
};

export const isAuthoritative = (contentStatus = '') => {
    return contentStatus === 'public_authoritative' ? true : false;
};

export const getDisplayName = (itemType = '', typeKeywords: string[]) => {
    ...
    if (itemType === 'Feature Service' || itemType === 'Feature Collection') {
        ...
        displayName =
            typeKeywords.indexOf('Elevation 3D Layer') > -1
                ? 'Elevation Layer'
                : 'Imagery Layer';
    ...
    // }
        ...
        if (
            ...
            (typeKeywords.indexOf('Hosted Service') > -1 ||
                typeKeywords.indexOf('Tiled') > -1) &&
...
export const getIconUrl = (type = '', typeKeywords: string[]) => {
    ...
    if (
        ...
        imgName =
            typeKeywords.indexOf('ViewingMode-Local') > -1
                ? 'webscenelocal'
                : 'websceneglobal';
    } else if (
        ...
        (itemType === 'explorer map' &&
            typeKeywords.indexOf('Explorer Document'))
    ) {
        imgName = 'mapsgray';
    } else if (
        ...
        itemType === 'arcgis pro add in' ||
        itemType === 'arcgis pro configuration'
    ) {
        ...
        (itemType === 'explorer map' &&
            typeKeywords.indexOf('Explorer Mapping Application') > -1) ||
        ...
        itemType === 'layer' ||
        itemType === 'layer package' ||
        itemType === 'explorer layer'
    ) {
        imgName = 'layersgray';
    } else if (itemType === 'scene package') {
        ...
        itemType === 'tile package' ||
        itemType === 'compact tile package'
    ) {
        imgName = 'tilepackage';
