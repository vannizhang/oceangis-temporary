...
export const getUrlForSearchOperation = (
    agolGroupId: string,
...
export default class GroupData {
    ...
    updateSelectedCategory(
        titleForSelectedMainCategory?: string,
        titlesForSelectedSubCategories?: string[]
    ) {
        ...
        this.categorySchema.categories.forEach((mainCategory) => {
            mainCategory.selected =
                titleForSelectedMainCategory &&
                titleForSelectedMainCategory === mainCategory.title
                    ? true
                    : false;
            ...
            mainCategory.categories.forEach((subcategory) => {
                subcategory.selected =
                    ...
                    titlesForSelectedSubCategories.indexOf(subcategory.title) >
                        -1
                        ? true
                        : false;
            });
    ...
    getCategoryPath() {
        ...
        const selectedMainCategory = this.categorySchema.categories.filter(
            (mainCategory) => {
        ...
        const selectedSubCategories = selectedMainCategory.categories.filter(
            (subcategory) => {
        ...
        if (
            ...
            selectedMainCategory.categories.length
        ) {
    ...
    private getQueryString(): string {
        const { searchTerm, contentType } = this.queryParams;

        const queryStrings = ['-typekeywords:"Requires Subscription"'];
    ...
    private getQueryParams({ start = 1, num = 10 } = {}): string {
    ...
    async search({ start = 1, num = 10 } = {}): Promise<SearchResponse> {
        const params = this.getQueryParams({ start, num });

        const urlForSearchOperation = getUrlForSearchOperation(
            this.AgolGroupId,
            this.AgolHost
        );
