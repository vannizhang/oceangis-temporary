...
const CategoryFilter: React.FC<Props> = ({
    ...
    const toggleMainCategory = (mainCategoryTitle: string) => {
        const newVal =
            mainCategoryTitle !== activeMainCategoryTitle
                ? mainCategoryTitle
                : '';
    ...
    const toggleAllSubcategories = (
        mainCategory: CategorySchemaMainCategory
    ) => {
        const allSubcategories = mainCategory.categories.map((d) => d.title);

        if (
            !selectedCategory ||
            mainCategory.title !== selectedCategory.title
        ) {
            setSelectedCategory({
    ...
    const getToggleSelectAllOption = (
        mainCategory: CategorySchemaMainCategory
    ) => {
        const selected =
            selectedCategory &&
        ...
        const filters = mainCategory.categories.map((subcategory, index) => {
            ...
            return (
                <div key={`subcategory-filter-${subcategory.title}`}>
                    <span
                        className={`${checkboxClass} font-size--2`}
                        onClick={toggleSubcategory.bind(
                            this,
                            mainCategory,
                            subcategory.title
                        )}
            ...
            return (
                ...
                >
                    <div
                        ...
                        onClick={toggleMainCategory.bind(
                            this,
                            mainCategory.title
                        )}
                    >
                    ...
                    {isActive ? getSubCategoryFilters(mainCategory) : null}
                </div>
            );
        });
    };
    ...
    const getChipsForSelectedCategories = () => {
        ...
        const chips = selectedCategory.subcategories.map(
            (subcategory, index) => {
                return (
                    <div
                    ...
                    >
                        ...
                        <span
                            className="icon-ui-close margin-left-half font-size--3"
                            style={{
                                cursor: 'pointer',
                            }}
                            onClick={removeFromSubcategories.bind(
                                this,
                                subcategory
                            )}
                        ></span>
